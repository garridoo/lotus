...
### hit1_1 genotype: root/rhizosphere/soil enriched OTUs

# subset samples

idx <- design$compartment %in% c("root", "soil", "rhizosphere") & 
       design$genotype %in% c("hit1_1", "soil") &
       design$soil=="CAS10"
...
pdf(file=paste(figures.dir, "hit1_root_rhizo_soil_enrichment_cas10.pdf", sep=""))

tern_e(df, prop_size=T, col=colors, grid_color="grey",
       labels_color="transparent", pch=19, main="hit1-1")
