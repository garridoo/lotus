...
enriched_otus_root <- data.frame(gifu_enriched=rownames(otu_table) %in% gifu_otus_root,
                            mutant_enriched=rownames(otu_table) %in% mutant_otus_root)
row.names(enriched_otus_root) <- rownames(otu_table)

write.table(enriched_otus_root, paste(results.dir, "gifu_v_mutant_root_OTUs.txt", sep=""),
...
### get mean R.A. in mutant root samples

idx <- design$compartment %in% c("root") &
       design$soil %in% c("CAS10") &
       design$genotype %in% c("hit1_1", "nfr5_2", "nfr5_3", "nin2")
root_mutant_samples <- design$SampleID[idx]
otu_table_norm_root_mutant <- otu_table_norm_all[, colnames(otu_table_norm_all) %in% root_mutant_samples]
...
gifu_mutant_pvals$enrichment <- gifu_mutant_pvals$otu %in% gifu_otus_root
...
enriched_taxa <- unique(taxonomy[taxonomy[, 1] %in% c(gifu_otus_root), 5])
...
ggsave(paste(figures.dir, "manhattan_gifu_v_mutant_root.pdf", sep=""), p1, width=10, height=3, useDingbats=F)

### mutant root-enriched OTUs
...
mutant_gifu_pvals$enrichment <- mutant_gifu_pvals$otu %in% mutant_otus_root
...
ra <- apply(otu_table_norm_root_mutant, 1, mean)
...
colors$colors <- as.character(colors$colors)
colors$colors[is.na(colors$colors)] <- "grey"
enriched_taxa <- unique(taxonomy[taxonomy[, 1] %in% c(mutant_otus_root), 5])
colors$colors[!colors$taxon %in% enriched_taxa] <- "grey"
...
ggsave(paste(figures.dir, "manhattan_mutant_v_gifu_root.pdf", sep=""), p1, width=10, height=3, useDingbats=F)
